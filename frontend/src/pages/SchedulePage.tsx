  ...
  return (
    <>
      ...
      {isLoading ? <Card><TableSkeleton /></Card> : !data || data.length === 0 ? (
        <Card><EmptyState title={t('schedule.noSchedule')} hint={t('schedule.noScheduleHint')} /></Card>
      ) : (
            ...
            return (
              <Card key={day} className={dayIndex === today ? 'ring-2 ring-blue-400' : ''}>
                <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
                  ...
                  {dayIndex === today && (
                    <span className="rounded-md bg-blue-50 px-2 py-0.5 text-xs font-bold text-blue-600">{t('schedule.today')}</span>
                  )}
                </div>
                <div className="space-y-2 p-4">
                  {slots.length === 0 ? (
                    <p className="py-2 text-center text-xs text-slate-400">{t('schedule.noLessons')}</p>
                  ) : slots.map((s) => (
                    <div key={s.id} className="rounded-lg border-l-4 border-blue-500 bg-slate-50 px-3 py-2">
                      ...
                      <div className="mt-0.5 text-xs text-slate-500">
                        {s.teacher_name ?? t('schedule.noTeacher')}{s.room ? ` · ${t('schedule.room')} ${s.room}` : ''}
                      </div>
                    </div>
                  ))}
                </div>
